...
import { checkouts, memoryDir, sideStore, syncProject } from "./link.js";
...
export function adoptMemory(repo: string, p: Project, ws: string, machine: string, check = false): number {
  ...
  for (const cand of candidatePaths(p, ws)) {
    ...
    ui.info(`${p.name}: adopting memory from ${contract(src)}`);
    ...
    if (!check) writeFileSync(join(dirname(src), "memory.adopted-by-cs"), `adopted into ${contract(dest)} on ${new Date().toISOString().slice(0, 10)}\n`);
  }
  if (!n) ui.ok(`${p.name}: no new memory to adopt`);
  return n;
}
export function adoptProjectFiles(repo: string, p: Project, ws: string, check = false) {
  const ch = syncProject(repo, p, ws, check); for (const c of ch) ui.step(`${p.name}: ${c}`); if (!ch.length) ui.ok(`${p.name}: nothing to adopt`); return ch.length;
}
...
export function adoptMcp(repo: string, p: Project, ws: string, check = false, show = false): number {
  ...
  const side = sideStore(repo, p); const f = join(side, ".mcp.json");
  ...
  if (Object.keys(secrets).length) { ui.warn(`${p.name}: values replaced by \${VAR} placeholders — store them: cs secrets set global ${Object.keys(secrets).map((k) => `${k}=…`).join(" ")}  (full values: cs adopt mcp ${p.name} --show)`); }
  return Object.keys(found).length;
}
export function runAdopt(repo: string, m: Machine, man: Manifest, what: string, names: string[], check: boolean, show: boolean) {
  ...
  for (const n of names) { const p = man.projects[n]; if (!p) throw new Error(`cs: unknown project '${n}'`);
    if (what === "memory") adoptMemory(repo, p, ws, m.name, check); else if (what === "project") adoptProjectFiles(repo, p, ws, check); else if (what === "mcp") adoptMcp(repo, p, ws, check, show); else throw new Error(`cs: unknown adopt target '${what}'`); }
}
