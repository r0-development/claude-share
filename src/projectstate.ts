/** Project state (see CONTEXT.md): <share>/projects/<name>/ ⇄ every unit of the project's checkout here (the root and each
 *  worktree; before it is a repo, the bare directory), newer wins per file. The auto-memory pointer in
 *  .claude/settings.local.json is this machine's: injected on the way into a unit, stripped on the way back, so the share
 *  stays machine-independent.
 *
 *  Three steps behind one silent entry point: `observe` (per-file stamps in the project state and in every unit, plus what
 *  was placed last time), `decide` (pure: what goes to the project state, what to which unit, what is removed, which exclude
 *  lines are missing), `write` (does it — or under `check` only says so — and returns the change lines). The module's
 *  second concern, the record of what was placed and the sweep that takes the pointer back out of a checkout whose project
 *  left the share, takes the same steps with `stripped` as its decision. Nothing here prints: quiet or not is the caller's
 *  ui.group. */
...
// ---------------------------------------------------------------- observe
/** One file as one place has it: its content with the pointer stripped (what newer-wins compares) and its mtime in seconds. */
export interface Stamp { data: Buffer; mtime: number }
/** A file in a unit also keeps the bytes on disk: the pointer is part of what must be right there. */
export interface OnDisk extends Stamp { raw: Buffer }
export interface Observation {
  state: Record<string, Stamp>;                       // the project state, rel → stamp (memory and secrets are not placed)
  targets: Record<string, Record<string, OnDisk>>;    // unit → rel → stamp: the files cs manages there and every file the project state has
  remembered: string[];                               // rels placed last time: one gone from the project state since is a deletion
...
/** The files cs manages in a unit: the root files and .claude/ minus what is that machine's (worktrees, plans, settings.json). */
...
function stateRels(state: string): Set<string> { const rels = new Set<string>(); walk(state, (rel) => rels.add(rel), (rel) => NOT_SYNCED.has(rel.split("/")[0])); return rels; }
...
export function observe(state: string, targets: string[], remembered: Iterable<string>, pointer: string): Observation {
  ...
  const o: Observation = { state: {}, targets: {}, remembered: [...remembered], pointer, excludes: [] };
  for (const rel of stateRels(state)) { const { data, mtime } = stampOf(join(state, rel), rel); o.state[rel] = { data, mtime }; }
  const seen = new Set<string>();
  for (const t of targets) {
    o.targets[t] = {};   // what the project state has is looked for in the unit too, managed or not: placed once, it must settle and flow back
    for (const rel of new Set([...managedRels(t), ...Object.keys(o.state)])) { const f = join(t, rel); if (existsSync(f) && statSync(f).isFile()) o.targets[t][rel] = stampOf(f, rel); }
...
export interface Decisions {
  toState: { rel: string; data: Buffer; mtime: number; from: string }[];      // newer content into the project state, from that unit
  toTargets: { target: string; rel: string; data: Buffer; mtime: number }[];  // into a unit, pointer included
  removals: { target: string; rel: string }[];                                // deleted in the project state → gone from the unit
...
export function decide(o: Observation): Decisions {
  const d: Decisions = { toState: [], toTargets: [], removals: [], excludes: [], placed: [] };
  const targets = Object.keys(o.targets); const remembered = new Set(o.remembered);
  const rels = new Set<string>([...Object.keys(o.state), SETTINGS_LOCAL]); for (const t of targets) for (const rel of Object.keys(o.targets[t])) rels.add(rel);
  for (const rel of [...rels].sort()) {
    const state = o.state[rel];
    let best = state?.data, mtime = state?.mtime ?? -1, from = "";
    ...
    if (!state && remembered.has(rel) && rel !== SETTINGS_LOCAL) {   // deleted in the project state since it was placed: the deletion travels, whatever a unit did to its copy
    ...
    if (substantive && (!state || !best.equals(state.data))) d.toState.push({ rel, data: best, mtime, from });
...
export function write(state: string, memory: string, d: Decisions, check = false): string[] {
  const lines: string[] = [];
  ...
  for (const rel of rels) {
    ...
    for (const s of d.toState) if (s.rel === rel) { lines.push(`project state ← ${rel} (from ${contract(s.from)})`); if (!check) put(join(state, rel), s.data, s.mtime); }
...
/** Strip the pointer from each unit and drop the project's record. One line per unit changed. */
function unplace(name: string, units: string[], why: string, check: boolean): string[] {
  const lines: string[] = [];
...
/** Place one project's state into its checkout here — every unit of it — with newer content flowing back. `[]` when the
 *  project has no checkout on this machine. Lines are not prefixed with the project's name. */
...
/** cs remove's tidy-up inside the units it leaves behind: the pointer goes, nothing else is touched; the record goes. */
export const forget = (name: string, units: string[]): string[] => unplace(name, units, "", false);
