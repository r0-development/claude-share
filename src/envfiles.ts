...
export interface EnvState {
  ...
  base?: Values; merge: Merge; example?: Values;
}
/** The merge of a state, by kind: values three-way per key, keys-only as a set with example defaults. */
export const mergeOf = (st: EnvState, decide: Partial<Record<string, Side>> = {}): Merge | KeysMerge =>
  st.kind === "local" ? mergeKeys(st.base && Object.keys(st.base), st.local ?? {}, Object.keys(st.stored ?? {}), st.example ?? {}) : merge3(st.base, st.local ?? {}, st.stored ?? {}, decide);
export const toFill = (st: EnvState): string[] => (st.kind === "local" ? (mergeOf(st) as KeysMerge).toFill : []);
...
function writeSnapshot(st: EnvState, values: Values) {
  const f = snapshotFile(st.project, st.file); if (st.kind === "local") values = blank(values);   // site-specific values stay in the file only
...
export async function applyEnv(repo: string, b: Backend, st: EnvState, decide: Partial<Record<string, Side>> = {}): Promise<{ stored: number; local: number; toFill: string[] }> {
