/** cs hooks install|remove|status: Claude Code hooks (in the config repo) + a 15-min timer. */
...
export function installHooks(repo: string, m: Machine, remove = false): boolean {
  ...
  if (changed) { writeFileSync(f, dumps(data)); git.git(["add", f], repo); git.commit(repo, `claude: ${remove ? "remove" : "install"} cs sync hooks`, "cs", `cs@${m.name}`); }
  return changed;
}
