...
import { identityMatches, type Project } from "./manifest.js";
...
export async function fix(share: Share): Promise<void> {
  ...
  for (const p of projects) {
    ...
    if (await ui.confirm(`${p.name} has no remote — create a private GitHub repo and push it?`, true)) await fixRemote(share, p);
  }
  for (const d of dirs) {
...
export async function runDoctor(share: Share, doFix = false, compact = false): Promise<number> {
  ...
  const ws = workspace(share); res.push(platform.isWSL() && ws.startsWith("/mnt/") ? ["fail", `workspace ${ws} is on the Windows filesystem; use the WSL home`] : ["ok", `workspace ${contract(ws)}`]);
  ...
  const ch: string[] = []; applySettings(share, true, ch); res.push(ch.length ? ["warn", "settings.json drift: " + ch.join("; ") + "  (cs apply)"] : ["ok", "settings.json rendered"]);
  ...
  const idr: R[] = [];
  for (const p of selectedProjects(share)) { const c = locate(p, ws); if (!present(c) && c.why !== "no remote") continue; const root = c.root;   // a remote-less repo still has an identity to check
