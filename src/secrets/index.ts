 ...
 *  `load` of a missing entry is undefined — a caller that wants values asks `values()`. */
import type { Share } from "../share.js";
import type { Values } from "../env.js";
import * as ui from "../ui.js";

/** A stored entry: its values, when it last changed and — when known — from which machine (what "take 2 keys from laptop" names). */
export interface Stored { values: Values; when: string; from?: string }
export interface SecretsStore {
  ...
  list(): Promise<string[]>;
  load(name: string): Promise<Stored | undefined>;
...
export const values = async (store: SecretsStore, name: string): Promise<Values> => (await store.load(name))?.values ?? {};
...
/** What `cs secrets init` / `status` say for none: nothing to set up, nothing to show. */
export const noneInfo = () => ui.info("secrets backend is 'none' — set [secrets].backend = \"sops\" in machine.toml to enable");
