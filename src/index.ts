...
program.command("status", HIDDEN).description("what bare `cs` shows").option("--fetch").option("--all").action(async (o) => { const { repo, m, man } = ctx(); const { runStatus } = await import("./status.js"); ui.intro(`cs status  ${ui.dim(m.name)}`); process.exitCode = await runStatus(repo, m, man, o.fetch, o.all); ui.outro(ui.dim("cs sync · cs clone · cs doctor")); });
...
async function main() {
  ...
  if (!argv.length) { if (machineExists()) { const finish = await startUpdateCheck(); const { repo, m, man } = ctx(); const { runStatus } = await import("./status.js"); ui.intro(`claude-share  ${ui.dim(m.name)}`); process.exitCode = await runStatus(repo, m, man); const behind = await Promise.race([finish(), new Promise<number>((r) => setTimeout(() => r(0), 50))]); ui.outro(behind > 0 ? ui.yellow(`cs is ${behind} commit(s) behind — run cs update`) : ui.dim("cs sync · cs new <project> --<identity> · cs --help")); return; } program.help(); }
