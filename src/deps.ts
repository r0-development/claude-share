...
const sh = async (cmd: string) => { const { shell } = await import("./proc.js"); const p = await shell(cmd); if (p.code !== 0) throw new Error(`command failed: ${cmd}\n${(p.err || p.out).split("\n").slice(-5).join("\n")}`); };
...
async function ageLinux() { const t = await latest("FiloSottile/age"); const tmp = join(home(), ".cache", "cs-age"); mkdirSync(tmp, { recursive: true }); const tgz = join(tmp, "age.tgz");
  await download(`https://github.com/FiloSottile/age/releases/download/${t}/age-${t}-linux-${a64()}.tar.gz`, tgz); await sh(`tar -xzf ${tgz} -C ${tmp}`); mkdirSync(BIN(), { recursive: true });
...
const brew = (pkg: string) => () => sh(`brew install ${pkg}`);
...
export async function runDeps(install = false, compact = false): Promise<number> {
  ...
  for (const [name, it] of Object.entries(CATALOG)) {
    ...
    if (install && inst) { try { await ui.spin(`installing ${name}…`, () => inst()); path = which(it.cmd); } catch (e: any) { ui.warn(`${name}: install failed: ${e.message}`); } }
