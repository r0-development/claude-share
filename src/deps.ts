...
const sh = (cmd: string) => { const p = spawnSync("bash", ["-lc", cmd], { stdio: "inherit" }); if (p.status !== 0) throw new Error(`command failed: ${cmd}`); };
...
export async function runDeps(install = false): Promise<number> {
  const rows: string[][] = []; let missingRequired = false; const apt: string[] = [];
  for (const [name, it] of Object.entries(CATALOG)) {
    let path = which(it.cmd);
    if (path) { rows.push([ui.green("✓"), name, ui.dim(ver(it.ver).slice(0, 40))]); continue; }
    ...
    if (path) { rows.push([ui.green("✓"), name, ui.dim("installed")]); continue; }
  ...
  ui.table(rows);
  if (apt.length) ui.info(ui.bold("sudo apt install -y " + apt.join(" ")));
  if (!install && rows.some((r) => !r[0].includes("✓"))) ui.info(ui.dim("cs deps --install  installs the user-local ones (age, sops, claude)"));
