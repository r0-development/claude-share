...
always end with a remote (creating a private GitHub repo if needed); `cs doctor` flags a workspace directory without
one and `--fix` offers to create the repo. Existing `kind` values are ignored and can be deleted.
